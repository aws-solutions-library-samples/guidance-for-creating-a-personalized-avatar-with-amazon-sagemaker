...
def handle(inputs: Input):
    ...
    subprocess.run(["/opt/djl/bin/s5cmd", "cp", output_file_name, f"s3://{s3_bucket}/{s3_prefix}/{os.path.basename(output_file_name)}"])
        
    # clean up
    shutil.rmtree(train_path.parent, ignore_errors=True)
    
    return Output().add_as_json({"status":status, "output_location": f"s3://{s3_bucket}/{s3_prefix}/{os.path.basename(output_file_name)}"})
